...
# Check if Ballerina is installed and resolve the executable
BAL_CMD=""
if command -v bal &> /dev/null; then
    BAL_CMD="bal"
elif which bal &> /dev/null; then
    BAL_CMD="bal"
elif where bal &> /dev/null 2>&1; then
    # On Windows Git Bash, .bat files aren't directly executable — use cmd /c
    BAL_CMD="cmd //c bal"
else
